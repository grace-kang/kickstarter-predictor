...
hyp_str = "blrbLen_" + str(MAX_BLURB_LENGTH) + "nLST_" + str(LSTM_UNITS) + \
          "lr_" + str(LEARNING_RATE) + "n_hidd_" + str(NUM_HIDDEN) + \
          "dOut_" + str(DROP_OUT_KEEP * 100) + "_"
...
def main():
    ...
    with open('blurb_data_300d.pickle', 'rb') as f:
    ...
    for i in range(ITERATIONS):
        ...
        if i % 1000 == 0 and i != 0: import numpy as np
...
def main():
    ...
    with tf.name_scope("LSTM_Cell") as scope:
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(LSTM_UNITS)
        lstm_cell = tf.contrib.rnn.DropoutWrapper(cell=lstm_cell, output_keep_prob=DROP_OUT_KEEP)
        stacked_lstm = tf.contrib.rnn.MultiRNNCell([lstm_cell for _ in range(NUM_HIDDEN)], state_is_tuple=True)
        initial_state = stacked_lstm.zero_state(BATCH_SIZE, dtype=tf.float32)
